...
use esp_idf_svc::{
    ...
    mqtt::client::{QoS, EspMqttClient, MqttClientConfiguration},
};
...
use thermostat::{ThermostatState, ThermostatEvent, ThermostatConfiguration, Device, ThermostatCommand};
...
fn process_thermostat_event<'a>(state: &'a mut ThermostatState, event: ThermostatEvent) {
                ...
                match event {
                    ThermostatEvent::MqttConnected => {
                        ...
                        state.command_topic_sub_id = state.client.subscribe(&state.command_topic, QoS::AtLeastOnce).ok();
                    },
                    ThermostatEvent::TopicSubscribed(id) => {
                        if let Some(state_id) = state.state_topic_sub_id {
                            if state_id == id {
                                ...
                                state.pub_id = mqtt::publish_configuration(&mut state.client, &state.config_topic, &state.configuration).ok();
                            }
                            ...
                            if command_id == id {
                                ...
                                state.state_topic_sub_id = state.client.subscribe(&state.state_topic, QoS::AtLeastOnce).ok();
                            }
                        }
                    },
...
fn main() -> Result<()> {
    ...
    let mut state = ThermostatState {
        ...
        pub_id: None,
        client: EspMqttClient::new_cb(&broker_url, &mqtt_config, move |ev| {mqtt::process_mqtt_event(ev, &event_tx)})?,
